...
use Fw2\Glimpse\Types\Entity\Attribute as BuilderAttribute;
...
beforeEach(function () {
    ...
    $this->ctxMock->shouldReceive('resolve')->andReturnUsing(fn($name) => "Fully\\Qualified\\{$name}");
});
...
it('uses context to resolve FQCN for attribute names', function () {
    ...
    $customCtx->shouldReceive('resolve')
        ->with('TestAttribute')
